...
def main(titleStream, contentdbPath, start=-1, end=1e100, doSegment=True):
...
if __name__ == "__main__":
    try:
        opts, args = getopt.getopt(sys.argv[1:], "s:e:i:d:", ["start=", "end=", "input=", 'db=', 'plain'])
    ...
    main(f, contentdbPath, start=start, end=end, doSegment=doSegment)
    if not f == sys.stdin:
        f.close()
